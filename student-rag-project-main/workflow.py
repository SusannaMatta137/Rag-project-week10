...
def rewrite_query(original_query, conversation_context=""):
    ...
    try:
        ...
        prompt = f"""Rewrite the user's question so it is clearer, more specific, and better suited for semantic search over technical documents.

{context_section}
Original question: {original_query}

Instructions:
- Resolve pronouns and vague references using the conversation context when provided
- Use precise technical wording that is likely to appear in knowledge-base documents
- Keep the same intent; do not answer the question
- Return only the rewritten question as a single line of text"""
...
def decompose_query(query):
    ...
    #
    try:
        prompt = f"""If the following question covers multiple distinct topics, split it into 2-3 simpler sub-questions.
If it is already a single simple question, return it unchanged as one line.

Question: {query}

Instructions:
- Return one sub-question per line
- Do not number the lines or add extra commentary
